...
public class DataIngestionValidatorService {
    ...
    private boolean validateRootLegalEntity(LegalEntity rootLegalEntity, LegalEntity createdLegalEntity,
        ...
        GetUsersByLegalEntityIdsResponse userList = userService
            .getUsersByLegalEntity(createdLegalEntity.getInternalId())
            .block();
        ...
        List<String> createdUserIds = userList.getUsers().stream().map(GetUserById::getExternalId)
    ...
    private boolean validateFunctionGroups(ServiceAgreement createdMsa,
        ...
        boolean missingData = false;
        List<SchemaFunctionGroupItem> createdFunctionGroups = accessGroupService
        ...
        List<String> createdFunctionGroupNames = createdFunctionGroups.stream().map(SchemaFunctionGroupItem::getName)
            .collect(Collectors.toList());
    ...
    private void validateProductGroup(ProductGroup productGroup) {
        ...
        productGroup.getCurrentAccounts().forEach(currentAccount -> {
            ArrangementItem createdArrangement = arrangementService.getArrangementByExternalId(currentAccount.getExternalId())
                .block();
        ...
        productGroup.getSavingAccounts().forEach(savingsAccount -> {
            ArrangementItem createdArrangement = arrangementService.getArrangementByExternalId(savingsAccount.getExternalId())
                .block();
            if (createdArrangement == null || createdArrangement.getId() == null) {
                log.error("Validation: Arrangement {} was not created",
